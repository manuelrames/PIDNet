...
from labels_railway import name2label
...
def createLabelImage(annotation, encoding, outline=None):
    ...
    # el orden en el que se pinta debe ser balasto y luego raíles!!!
    #ballast_annot = [annot for annot in annotation.objects if annot.label == 'ballast']
    #train_rail_annot = [annot for annot in annotation.objects if annot.label == 'train_rail']
    #annotation.objects = ballast_annot + train_rail_annot
    no_irrig_zone_annot = [annot for annot in annotation.objects if annot.label == 'no_irrig_zone']
    irrig_zone_annot = [annot for annot in annotation.objects if annot.label == 'irrig_zone']
    other_rail_annot = [annot for annot in annotation.objects if annot.label == 'other_rail']
    right_rail_annot = [annot for annot in annotation.objects if annot.label == 'right_rail']
    left_rail_annot = [annot for annot in annotation.objects if annot.label == 'left_rail']
    annotation.objects = no_irrig_zone_annot + irrig_zone_annot + other_rail_annot + right_rail_annot + left_rail_annot
